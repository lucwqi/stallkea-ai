    ...
    <div class="flex-1 overflow-y-auto scrollbar-hide">
        ...
        <div class="pt-2 pb-4 border-b border-gray-800/40">
          <div class="flex overflow-x-auto gap-4 px-4 scrollbar-hide pb-2">
            ...
            <!-- Unlocked Stories -->
            <button 
              v-for="(story, index) in unlockedStories" 
              :key="story.username" 
              class="flex flex-col items-center gap-1.5 min-w-[72px]"
              @click="openBlockedModal"
            >
              <div :class="isCloseFriend(story.username) ? 'bg-[#1cd24a]' : 'bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-500'" class="p-[2px] rounded-full">
            ...
            <div v-for="postItem in posts" :key="postItem.post.id" class="flex flex-col mb-4 border-b border-gray-900 pb-2">
                ...
                <div class="flex items-center justify-between px-3 py-2">
                    <div class="flex items-center gap-2">
                        <div :class="isCloseFriend(postItem.de_usuario.username) ? 'bg-[#1cd24a]' : 'bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-500'" class="p-[2px] rounded-full">
                             <div class="bg-black p-[2px] rounded-full">
                                <img 
...
const isCloseFriend = (username: string) => {
    const cfUsernames = stories.value.slice(0, 3).map(s => s.username)
    return cfUsernames.includes(username)
}
